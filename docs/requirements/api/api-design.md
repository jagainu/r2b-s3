...
| 6 | GET | `/auth/csrf` | CSRFトークンを取得（非HttpOnly Cookie + レスポンスボディ） | 🌐 | - |
| 7 | GET | `/auth/me` | 自分のユーザー情報取得 | 🔒 | - |
| 8 | GET | `/cat-breeds` | 猫種一覧（フィルタ対応） | 🌐 | - |
| 9 | GET | `/cat-breeds/{id}` | 猫種詳細（写真・特徴込み） | 🌐 | - |
| 10 | GET | `/cat-breeds/{id}/similar` | 類似猫一覧 | 🌐 | - |
| 11 | GET | `/masters/coat-colors` | 毛色マスター一覧 | 🌐 | - |
| 12 | GET | `/masters/coat-patterns` | 模様マスター一覧 | 🌐 | - |
| 13 | GET | `/masters/coat-lengths` | 毛の長さマスター一覧 | 🌐 | - |
| 14 | GET | `/quiz/today` | 今日の一匹を取得 | 🔒 | - |
| 15 | POST | `/quiz/sessions` | クイズセッション生成（全問題をDB保存） | 🔒 | ✅ |
| 16 | POST | `/quiz/answer` | クイズ回答を送信（サーバー判定） | 🔒 | ✅ |
| 17 | POST | `/quiz/sessions/{session_id}/finalize` | セッション完了・結果をサーバー算出して保存 | 🔒 | ✅ |
| 18 | GET | `/users/me/stats` | ユーザー統計（累計覚えた種類数）取得 | 🔒 | - |
| 19 | GET | `/users/me/sessions/latest` | 最新セッション結果取得（?source=quiz または today） | 🔒 | - |
...
CSRFトークンを配布する（二重送信Cookie方式）。フロントエンドはページ初期化時またはログイン後にこのエンドポイントを呼び出す。
...
**副作用**：レスポンスヘッダーで非HttpOnly Cookie を発行。
```
Set-Cookie: csrf_token=<token>; Secure; SameSite=None; Path=/; Max-Age=3600
```

**フロントエンドの使い方**：
1. このエンドポイントからトークンを取得（Cookie にも同じ値がセットされる）
2. 状態変更リクエスト（POST/PUT/DELETE）時に `X-CSRF-Token: <token>` ヘッダーを付与
